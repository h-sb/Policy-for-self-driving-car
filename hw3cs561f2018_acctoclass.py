...
class mdpinit:
    ...
    def ReadInput(self):
        self.orientations = NORTH, SOUTH, EAST, WEST = [(1, 0), (0, -1), (-1, 0), (0, 1)]
        self.turns = self.LEFT, self.RIGHT = (+1, -1)
        self.start_pos_car=[]
        self.end_pos_car=[]
        self.obs_pos=[]
        grids_list=[]
    ...
    def turn_heading(self,heading, inc, headings=None):
        if headings is None:
            headings=self.orientations
        return headings[(headings.index(heading) + inc) % len(headings)]
    def turn_right(self, heading):
        return self.turn_heading(heading, self.RIGHT)
    def turn_left(self,heading):
        return self.turn_heading(heading, self.LEFT)
